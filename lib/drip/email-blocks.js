...
export function sampleBoxHtml(sample) {
  if (!sample?.url) return "";
  return `
    <div style="margin:0 0 20px;padding:16px;border:2px solid #0A2540;border-radius:12px;background:#F8FAFC">
      <p style="margin:0 0 4px;font-size:11px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:#64748b">Ilmainen näyte</p>
      <p style="margin:0 0 8px;font-weight:700;color:#0A2540">${sample.title || "Kokeile ilmaiseksi"}</p>
      <p style="margin:0;font-size:13px;color:#64748b;line-height:1.5">Ei rekisteröitymistä — voit tutustua heti.</p>
    </div>`;
}

export function compactOfferLinkHtml(payload, checkoutUrl) {
  if (!payload?.priceEur || !checkoutUrl) return "";
  return `
    <p style="margin:0 0 20px;font-size:14px;color:#475569;text-align:center;line-height:1.5">
      Henkilökohtainen tarjouksesi: <strong>${payload.priceEur} €</strong> —
      <a href="${checkoutUrl}" style="color:#0A2540;font-weight:700">siirry kassalle</a>
    </p>`;
}
