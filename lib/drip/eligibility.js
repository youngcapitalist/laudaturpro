...
async function hasPaidValintakoeProfile(email) {
  ...
  let user = await tryEmail(normalized);
  if (!user?.id && emailKey !== normalized) {
    user = await tryEmail(emailKey);
  }
...
/** Maksava asiakas missä tahansa tuotteessa (LaudaturPro tai valintakoekurssit). */
export async function isPayingCustomerAnywhere(email) {
  const emailKey = canonicalEmailKey(email);
  const normalized = normalizeEmail(email);
  if (await hasLaudaturAccessForEmail(normalized, emailKey)) return true;
  return hasPaidValintakoeProfile(email);
}

/** Onko jo maksanut — ei lähetetä follow-upia. */
export async function isCustomerForStream(email, stream) {
  // Follow-up vain, jos ei ole maksava asiakas missään tuotteessa.
  if (stream === "laudaturpro" || stream.startsWith("valintakoe_")) {
    return isPayingCustomerAnywhere(email);
  }
  return false;
}
