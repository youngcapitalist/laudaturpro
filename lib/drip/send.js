 ...
 */
function refreshOfferPayload(payload, step, streamId) {
  if (streamId !== "laudaturpro") return payload;
...
export async function sendDripEmail({ email, stream, stepIndex, payload }) {
  ...
  const p = refreshOfferPayload(payload || {}, step, streamConfig.id);
  ...
  const cta = resolveStepField(step.cta, p) || "Avaa tarjoukseni";
...
<body style="margin:0;background:#f4f6f8;font-family:system-ui,sans-serif;color:#0A2540">
  <div style="max-width:560px;margin:0 auto;padding:24px 16px">
    ...
    <div style="background:#fff;border-radius:0 0 16px 16px;padding:24px;border:1px solid #e5e7eb;border-top:0">
      ...
      ${bulletsHtml(bullets)}
      ${urgencyHtml(urgency)}
      <a href="${checkout}" style="display:block;text-align:center;background:#D4AF37;color:#0A2540;font-weight:800;text-decoration:none;padding:16px 24px;border-radius:999px;font-size:15px">${cta}</a>
